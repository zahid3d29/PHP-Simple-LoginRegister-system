<?php
    ...
    if ( isset($_POST['signup-submit'])){
        ...
        if( empty($username) || empty($email) || empty($password) || empty($passwordRepeat)){
            ...
            header("Location: ../signup-form.php?error=invaliduid&mail");
            exit();
        } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            header("Location: ../signup-form.php?error=invalidmail&uid=" . $username);
            exit();
        } elseif (!preg_match("/[a-zA-Z0-9]*$/", $username)){
            header("Location: ../signup-form.php?error=invaliduidmail&mail=" . $email);
            exit();
            ...
            if(!mysqli_stmt_prepare($stmt, $sql)){
                ...
                if($resultCheck > 0 ){
                    header("Location: ../signup-form.php?error=usertaken&mail=" .$email);
                    exit();
                } else{
